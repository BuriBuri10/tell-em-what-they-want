...
# class UserSegmenter:
#     """
#     Node responsible for classifying users into segments using inline prompts.
#     """

#     def __init__(self):
#         """
#         Initializes the user segmentation chain with inline prompt and structured output.
#         """
#         self.chain = ChainAccess.get_orchestrator().build(
...
#         logger.info("UserSegmenter initialized with inline prompt chain.")
...
#             prompt_text = f"""
#             You are an expert customer segmentation model. Given a user query, classify them into one of the segments: "budget", "standard", or "premium".

#             Respond strictly in the following JSON format:

#             {{{{ 
#             "segment": "<one of: budget | standard | premium>" 
#             }}}}
...
#             result: SegmentOutput = await self.chain.ainvoke({"input": prompt_text})
...
#         sample_input = "stech products"

#         segmenter = UserSegmenter()
#         # for q in sample_inputs:
#         state = GraphState(query=sample_input, vdb_query=sample_input, conversation_history=[])
#         updated_state = await segmenter.process(state)
#         print(f"Input: {sample_input} -> Segment: {updated_state.segment}")
